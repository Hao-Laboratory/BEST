...
specs_train = torch.zeros([TrainingDataSize, 1], device=device_data, dtype=dtype)
tf_train = torch.zeros([TrainingDataSize, TFNum], device=device_data, dtype=dtype)
specs_test = torch.zeros([TestingDataSize, 1], device=device_test, dtype=dtype)
tf_test = torch.zeros([TestingDataSize, TFNum], device=device_test, dtype=dtype)
data = h5py.File(path_data + 'centerWL_gaus_020.mat', 'r')
Specs_all = np.array(data['centerWL_gaus'])
...
specs_test[0:TestingDataSize, :] = torch.tensor(
    Specs_all[: TestingDataSize, :])
...
for k in range(len(lr)):
 ...
 #  hsnet.add_module('LReLU1', nn.LeakyReLU())
    ...
    #hsnet.add_module('Linear6', nn.Linear(Layer3Num, 500))
    #hsnet.add_module('LReLU6', nn.LeakyReLU())
    #hsnet.add_module('Linear7', nn.Linear(Layer3Num, 500))
    #hsnet.add_module('LReLU7', nn.LeakyReLU())
    hsnet.add_module('Linear8', nn.Linear(500, 1))
   ...
   # tf_train = torch.mm(specs_train, specs_filter)
    Output_temp = hsnet(tf_train[0:100000, :].to(device_test).unsqueeze(0)).squeeze(0)  # 只有一组数据，使用unsqueeze将其增加一维以使BatchNorm不报错
    FigureTrainLoss = MatchLossFcn(specs_train[0:100000, :].to(device_test), Output_temp)
    plt.figure()
    plt.plot(specs_train[0:100000, :].cpu().numpy())
    plt.plot(Output_temp[:,:].detach().cpu().numpy())
   # plt.ylim(0, 1)
    plt.legend(['GT', 'pred'], loc='upper right')
    plt.savefig(path + 'train')
    # plt.show()
    scio.savemat(path + 'datatest.mat', {'centerdata': Output_temp.detach().cpu().numpy()})
   # tf_test = torch.mm(specs_test, specs_filter)
    Output_temp = hsnet(tf_test[0:100000, :].to(device_test).unsqueeze(0)).squeeze(0)  # 只有一组数据，使用unsqueeze将其增加一维以使BatchNorm不报错
    FigureTestLoss = MatchLossFcn(specs_test[0:100000, :].to(device_test), Output_temp)
    plt.figure()
    plt.plot(specs_test[0:100000, :].cpu().numpy())
    plt.plot(Output_temp[:,:].detach().cpu().numpy())
   # plt.ylim(0, 1)
    plt.legend(['GT', 'pred'], loc='upper right')
    plt.savefig(path + 'test')
    # plt.show()
    scio.savemat(path + 'datatrain.mat', {'centerdata': Output_temp.detach().cpu().numpy()})
    print('Training finished!',
          '| loss in figure \'train.png\': %.12f' % FigureTrainLoss.data.item(),
          '| loss in figure \'test.png\': %.12f' % FigureTestLoss.data.item())
    print('Training finished!',
          '| loss in figure \'train.png\': %.12f' % FigureTrainLoss.data.item(),
          '| loss in figure \'test.png\': %.12f' % FigureTestLoss.data.item(), file=log_file)
    log_file.close()
